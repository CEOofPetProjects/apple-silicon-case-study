...
# opening DB connection, extracting data (base, Pro, Max only), loading it into DF and closing connection
...
WHERE c.chip_name NOT LIKE '%Ultra%' AND c.chip_name NOT LIKE '%A18%'
...
print("\nSpec profiles per generation (base, Pro, Max only)")
...
plt.tight_layout()
plt.savefig("visualizations/2_2_specs_base_pro_max.png", dpi=300)
plt.close()